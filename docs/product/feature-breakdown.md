...
**Assumption:** run numbering (05-14) is proposed here, not given in the
...
(database, settings persistence, error handling, logging, clock) turned
out to need its own dedicated run and was inserted as Run 06 — every run
from the old "Run 06" (Water) onward shifted down by one. The list below
reflects the actual executed sequence.
...
- Onboarding flow shell: language select → module select → dashboard,
  with each module's setup step stubbed as "coming in Run 07/08/10."
- Module-plugin contract (interface only): the shared contract each
  ...
  07, not speculatively designed in the abstract first.

## Run 06 — Core infrastructure

- Database: Drift, common tables only (`app_settings`,
  `notification_ledger`, `achievements`), migration scaffolding from
  schema v1.
- Settings repository: theme/locale (and future PIN/water-unit fields)
  backed by the DB, replacing Run 05's in-memory seam — theme/language
  now survive app restart.
- Error handling: `AppException`/`Result<T>` taxonomy, top-level error
  widget + zone guard.
- Logging: leveled `logger` setup, rotating on-disk log file, redaction
  helper for the "type + id only, never content" rule.
- Clock abstraction: injected `package:clock` everywhere instead of
  `DateTime.now()`, plus local-day bucketing helpers every module's
  streak/schedule logic depends on.
...
- Notification integration: scheduled exact alarms per dose instance,
  ...
  action component also used by Prayer in Run 11).
...
- Notification integration: reuses the Run 09 notification-action
...
- Local export/import (file-based backup) — candidate for v1.1, not a
  Run 05-14 unit; flagged here so it isn't accidentally half-built during
