...
class RecalibrationRepository {
  ...
  Future<RecalibrationMarkerRow> forModule(String moduleId) async {
    final row = await (_db.select(_db.recalibrationMarkersTable)
    ...
    final settingsRow = await (_db.select(_db.appSettingsTable)
          ..where((t) => t.id.equals('singleton'))
          ..limit(1))
        .getSingleOrNull();
    final installAt = settingsRow?.installDate ??
